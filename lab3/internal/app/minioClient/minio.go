...
func UploadImage(ctx context.Context, client *minio.Client, bucket string, file *multipart.FileHeader, device ds.Device) (string, error) {
	...
	//objectName := fmt.Sprintf("%s%s", device.Name, ext)
	randomPart := generateRandomString(10) // abc123def4
	objectName := fmt.Sprintf("device_%d_%s%s", device.Device_ID, randomPart, ext)
...
// ФУНКЦИЮ ДЛЯ ГЕНЕРАЦИИ СЛУЧАЙНОЙ СТРОКИ
